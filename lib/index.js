...
// Log
Pkgm.prototype.log = function(type) {
    this.emit("log", {
        'type': type,
...
Pkgm.prototype.remove = function(pkg) {
    ...
    delete this.packages[pkg.pkg.name];
};
...
// Bundle all packages into one files
Pkgm.prototype.bundleAll = function(output) {
    var that = this;
    var fd;
    var pkgs = this.orderedPackages(pkgs);

    // CLear file
    return Q.nfcall(fs.writeFile, output, "", { encoding: "utf-8" })
    .then(function() {
        return Q.nfcall(fs.open, output, 'a', 777);
    })
    .then(function(_fd) {
        fd = _fd;
        ...
        return _.reduce(pkgs, function(prev, pkg, name) {
            return prev.then(function() {
                return Q.nfcall(fs.readFile, pkg.buildOutput())
                .then(function(buf) {
                    return Q.nfcallfs.write(fd, buf, null);
                });
            });
        }, Q());
    })
