...
Package.prototype.run = function(context) {
    ...
    .fail(function(err) {
        if (err.code == "MODULE_NOT_FOUND" && !that.pkg.main) return Q();
        throw err;
    });
};

// Return filepath to the build output
Package.prototype.buildOutput = function() {
    return path.resolve(this.root, BUILD_NAME);
}
...
Package.prototype.optimizeClient = function(force) {
    ...
    .then(function(src) {
        if (that.manager.options.uglify) {
            src = UglifyJS.minify(src.toString("utf8"), {fromString: true}).code;
        }
